<template>
  <div class="group relative premium-blur rounded-3xl p-6 transition-all duration-500 hover:shadow-2xl hover:shadow-primary/10 active:scale-[0.98] hover:translate-y-[-4px]">
    ...
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="text-[10px] font-mono text-slate-500 bg-slate-950/50 px-2 py-1 rounded-lg border border-slate-800">
    ...
    <p class="text-slate-400 text-sm line-clamp-2 mb-6 leading-relaxed">
      {{ ticket.descripcion }}
    </p>
    ...
    <div class="flex items-center justify-between pt-4 border-t border-slate-800/50">
      <div class="flex items-center gap-3">
        <div class="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-xs font-bold text-primary">
          {{ (ticket.usuarioNombre || 'U')[0] }}
        </div>
        <div class="text-xs">
          <div class="text-slate-300 font-semibold">{{ ticket.usuarioNombre || 'Usuario' }}</div>
