  ...
  <div class="min-h-screen text-slate-200 flex font-['Outfit'] select-none transition-colors duration-700">
    ...
    <aside class="w-20 lg:w-64 bg-slate-900/20 border-r border-white/5 flex flex-col transition-all duration-500 backdrop-blur-3xl sticky top-0 h-screen z-40">
      <div class="p-6 flex items-center gap-4">
        ...
        <span class="text-xl font-black text-white tracking-tighter hidden lg:block uppercase italic">Mi Perfil</span>
      </div>

      <nav class="flex-1 px-4 py-8 space-y-2">
        <router-link :to="authStore.isOperador ? '/admin' : '/dashboard'" class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 transition-all group">
        ...
        <router-link to="/settings" class="flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-400 hover:text-white hover:bg-white/5 transition-all group">
    ...
    <main class="flex-1 min-w-0 h-screen overflow-y-auto page-fade-in custom-scrollbar">
      <header class="h-20 flex items-center px-10 bg-white/[0.01] backdrop-blur-xl border-b border-white/[0.02] sticky top-0 z-30">
        <h2 class="text-2xl font-black text-white tracking-tight uppercase italic opacity-90">Mi Perfil</h2>
      </header>

      <div class="max-w-3xl mx-auto py-16 px-10 space-y-8 page-fade-in">

        <!-- Avatar Card -->
        <div class="premium-blur rounded-[2.5rem] p-10 flex flex-col sm:flex-row items-center gap-10">
          <!-- Big Avatar -->
          <div :class="`relative w-32 h-32 rounded-3xl bg-gradient-to-br ${settingsStore.themeClasses} flex items-center justify-center text-5xl font-black text-white shadow-2xl select-none`">
          ...
          <div class="text-center sm:text-left">
            <h1 class="text-4xl font-black text-white tracking-tighter">{{ authStore.user?.nombre }}</h1>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div class="premium-blur rounded-3xl p-8">
          ...
          <div class="premium-blur rounded-3xl p-8">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span class="text-sm font-bold text-white">Ir al Dashboard</span>
              </router-link>
              <router-link to="/settings" class="flex items-center gap-4 p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-all group">
                <SettingsIcon class="w-5 h-5 text-primary group-hover:scale-110 transition-transform" />
                <span class="text-sm font-bold text-white">Configuración del Sistema</span>
              </router-link>
            </div>
          </div>
        </div>
