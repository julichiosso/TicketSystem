<template>
 <div class="min-h-screen bg-slate-50 text-slate-900 p-10 font-sans">
 <div class="max-w-4xl mx-auto">
 <div class="flex items-center gap-4 mb-12">
 <button @click="router.back()" class="p-3 bg-white/5 hover:bg-slate-50 text-slate-500 rounded-2xl transition-all">
 <ArrowLeftIcon class="w-6 h-6" />
 </button>
 <h2 class="text-4xl font-black text-slate-900 italic tracking-tighter uppercase">Configuración</h2>
 </div>

 <div class="space-y-12">
 <!-- Theme Section -->
 <section class="bg-white border border-slate-200 rounded-[3rem] p-10 ">
 <div class="flex items-center gap-4 mb-8">
 <PaletteIcon class="w-6 h-6 text-blue-500" />
 <h3 class="text-xl font-black text-slate-900 uppercase italic">Apariencia</h3>
 </div>

 <div class="grid grid-cols-2 sm:grid-cols-5 gap-6">
 <button 
 v-for="color in ['light','blue', 'indigo', 'emerald', 'rose']" 
 :key="color"
 @click="settingsStore.setThemeColor(color)"
 class="group relative h-32 rounded-3xl border-2 transition-all overflow-hidden"
 :class="[
 settingsStore.themeColor === color ? 'border-primary scale-[1.05]' : 'border-slate-200 hover:border-slate-200'
 ]"
 >
 ...
 <div class="absolute bottom-3 left-0 w-full text-center text-[8px] font-black uppercase tracking-widest text-slate-900/40">
 ...
 <h3 class="text-xl font-black text-slate-900 uppercase italic">Densidad de Interfaz</h3>
 </div>
 ...
 :class="settingsStore.uiDensity === 'comfortable' ? 'bg-primary border-primary text-slate-900' : 'bg-white/5 border-slate-200 text-slate-500'"
 >
 ...
 class="flex-1 py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all border-2"
 :class="settingsStore.uiDensity === 'compact' ? 'bg-primary border-primary text-slate-900' : 'bg-white/5 border-slate-200 text-slate-500'"
 >
 ...
 <h3 class="text-xl font-black text-slate-900 uppercase italic">Sistema</h3>
 </div>

 <div class="space-y-4">
 <div class="flex items-center justify-between p-6 bg-white/[0.02] rounded-3xl border border-slate-200">
 <div>
 <p class="text-sm font-bold text-slate-900">{{ i18n[settingsStore.language]?.notifications }}</p>
 ...
 class="w-12 h-6 rounded-full relative cursor-pointer transition-all border border-slate-200"
 :class="settingsStore.notificationsEnabled ? 'bg-primary/30' : 'bg-white'"
 >
 <div 
 ...
 <div class="p-6 bg-white/[0.02] rounded-3xl border border-slate-200">
 <div class="flex items-center justify-between mb-4">
 <div>
 <p class="text-sm font-bold text-slate-900">{{ i18n[settingsStore.language]?.language }}</p>
 ...
 :class="settingsStore.language === code ? 'bg-primary/20 border-primary text-slate-900' : 'bg-white/5 border-slate-200 text-slate-500'"
 >
 {{ label }}
 </button>
 </div>
...
const i18n = {
 ...
 light: 'Claro', blue: 'Azul', indigo: 'Índigo', emerald: 'Esmeralda', rose: 'Rosa'
 },
 en: {
 ...
 light: 'Light', blue: 'Blue', indigo: 'Indigo', emerald: 'Emerald', rose: 'Rose'
 },
 fr: {
 ...
 light: 'Clair', blue: 'Bleu', indigo: 'Indigo', emerald: 'Émeraude', rose: 'Rose'
 }
};
</script>
